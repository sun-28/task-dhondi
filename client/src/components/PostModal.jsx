...
const Modal = () => {
    ...
    const handleTagDelete = (tagToDelete) => {
        setpostDetails({
            ...postDetails,
            tags: postDetails.tags.filter(tag => tag !== tagToDelete)
        });
    };
                        ...
                        <div className="relative my-6 w-2/5 min-w-[300px] bg-white p-4 rounded-xl">
                            <div className="flex flex-col w-full h-full gap-4 ">
                                ...
                                <div className="flex items-center flex-wrap">
                                            <span className="bg-purple-50 text-purple-600 font-normal rounded px-1 h-4 text-xs m-1">
                                                 Tags
                                            </span>
                                        {postDetails && postDetails.tags.map((tag, idx) => (
                                            <span onClick={() => handleTagDelete(tag)} key={postDetails.id} className="bg-purple-50 text-purple-600 font-normal rounded px-1 h-4 text-xs m-1">
                                                {tag}
                                            </span>
                                        ))}
                                        {showTagInput && (
